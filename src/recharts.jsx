  ...
  return (
    <div style={{ position: 'relative', width: '100%', height: '100%' }}>
      <svg viewBox={`0 0 ${width} ${height}`} width="100%" height="100%">
        <line x1={padding} y1={height - padding} x2={width - padding} y2={height - padding} stroke="#d9e4dc" />
        <line x1={padding} y1={padding} x2={padding} y2={height - padding} stroke="#d9e4dc" />
        {lines.map((line) => {
          const points = data
            .map((row, idx) => {
        ...
        {data.map((row, idx) => (
          <text key={idx} x={scaleX(idx)} y={height - 8} fontSize="10" textAnchor="middle" fill="#5c6b63">{row[xKey]}</text>
        ))}
        {yComp?.props?.unit ? <text x={5} y={15} fontSize="11" fill="#5c6b63">{yComp.props.unit}</text> : null}
      </svg>
...
export function Pie({ data = [], dataKey = 'value', nameKey = 'name', outerRadius = 80 }) {
  ...
  return <>
    {data.map((item, idx) => {
      ...
      return <path key={idx} d={path} fill={["#1b4332","#2b6cb0","#b7791f","#c53030"][idx%4]}><title>{item[nameKey]}: {item[dataKey]}</title></path>;
    })}
  </>;
}
Pie.displayName='Pie';
...
export function BarChart({ data = [], children }) {
  ...
  return <svg viewBox={`0 0 ${width} ${height}`} width="100%" height="100%">
    <line x1={p} y1={height-p} x2={width-p} y2={height-p} stroke="#d9e4dc" />
    ...
    {data.map((row,i)=><text key={i} x={x(i)} y={height-8} fontSize="10" fill="#5c6b63">{row.mes||row.nome||i+1}</text>)}
  </svg>;
}
