...
function validarForm(form, qtdAtual) {
  ...
  if (!form.tipo) return 'Selecione o tipo de saída.';
  if (!form.data) return 'Informe a data.';
  if (qtd <= 0) return 'Informe uma quantidade válida.';
  if (qtd > Number(qtdAtual || 0)) return 'Quantidade maior do que o saldo atual do lote.';
  if (Number(form.peso_medio || 0) <= 0) return 'Informe o peso médio na saída.';
  if (form.tipo === 'venda' && Number(form.preco_arroba || 0) <= 0) return 'Informe o preço por arroba.';
  if (!String(form.destino || '').trim()) return 'Informe comprador/destino.';
  return null;
}
