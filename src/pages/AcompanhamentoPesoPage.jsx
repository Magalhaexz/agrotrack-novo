...
export default function AcompanhamentoPesoPage({ db }) {
  ...
  const [loteSelecionado, setLoteSelecionado] = useState(lotes.length ? String(lotes[0].id) : '');
  ...
  const resumo = useMemo(() => {
    ...
    const ganhoTotal = Number(ultima.peso_medio || 0) - Number(primeira.peso_medio || 0);
      ...
      <div className="kpi-grid-3 kpi-grid-3--compact">
        <div className="kpi-card kpi-card--compact">
          ...
          <div className="kpi-value">
            {resumo.primeiraPesagem ? `${formatarNumero(resumo.primeiraPesagem.peso_medio)} kg` : '—'}
          </div>
          <div className="kpi-sub">{resumo.primeiraPesagem ? formatarData(resumo.primeiraPesagem.data) : 'sem dados'}</div>
        </div>

        <div className="kpi-card kpi-card--compact">
          <div className="kpi-label">Última pesagem</div>
          <div className="kpi-value">
            {resumo.ultimaPesagem ? `${formatarNumero(resumo.ultimaPesagem.peso_medio)} kg` : '—'}
          </div>
          <div className="kpi-sub">{resumo.ultimaPesagem ? formatarData(resumo.ultimaPesagem.data) : 'sem dados'}</div>
        </div>

        <div className="kpi-card kpi-card--compact">
          <div className="kpi-label">Ganho total</div>
          <div className="kpi-value">{pesagensLote.length ? `${formatarNumero(resumo.ganhoTotal)} kg` : '—'}</div>
          <div className="kpi-sub">
            {resumo.dias > 0 ? `${resumo.dias} dias · GMD ${formatarNumero(resumo.gmd)} kg/dia` : 'dados insuficientes'}
          </div>
        </div>
      </div>
      ...
      <div className="grid-2">
        <div className="fazendas-card">
          ...
          <div className="card-body">
            <PesoChart data={pesagensLote} metaGmd={loteAtual?.gmd_meta || 0} />
          </div>
        </div>
          ...
          <div className="card-body">
            <div className="peso-summary-grid">
              ...
              <div className="peso-summary-card">
                ...
                <div className="peso-summary-desc">entre 1ª e última pesagem</div>
              </div>

              <div className="peso-summary-card">
                <div className="peso-summary-value">{formatarNumero(resumo.gmd)}</div>
                <div className="peso-summary-label">GMD</div>
                <div className="peso-summary-desc">kg por dia</div>
              </div>

              <div className="peso-summary-card">
                <div className="peso-summary-value">{loteAtual?.gmd_meta ? formatarNumero(loteAtual.gmd_meta) : '0,00'}</div>
            ...
            <table className="data-table">
              ...
              <tbody>
                {pesagensLote.map((item, index) => {
                  ...
                  const variacao = anterior ? Number(item.peso_medio || 0) - Number(anterior.peso_medio || 0) : null;
