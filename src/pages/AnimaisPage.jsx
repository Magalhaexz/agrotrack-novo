...
export default function AnimaisPage({ db, setDb, onConfirmAction }) {
  ...
  return <div className="page animais-page">
    <section className="animais-hero"><div><h1>Animais</h1><p>Cadastro organizado para grupos, individuais e movimentacoes.</p></div><Button icon={<Plus size={16} />} onClick={abrirNovo}>Novo cadastro</Button></section>

    <div className="dashboard-grid dashboard-grid--kpi-main">
      <Card title="Total de cabecas"><div className="animais-kpi-value">{resumo.totalCabecas}</div></Card>
    ...
    <Card title="Cadastros" subtitle="Visualizacao simplificada.">
      <div className="segmented-control" role="tablist" aria-label="Abas de animais">
        ...
        <button type="button" className={`segment ${abaAtiva === 'movimentacoes' ? 'active' : ''}`} onClick={() => setAbaAtiva('movimentacoes')}>Movimentacoes</button>
      </div>
      {abaAtiva !== 'movimentacoes' && (<div className="table-responsive"><table className="data-table"><thead><tr><th>Identificacao/Lote</th><th>Fazenda/Lote</th><th>Quantidade</th><th>Peso inicial</th><th>Peso atual</th><th>Status</th><th>Acoes</th></tr></thead><tbody>{listaAtiva.map((animal) => <tr key={animal.id}><td>{animal.identificacao} / {animal.loteNome}</td><td>{lotesMap.get(Number(animal.lote_id))?.fazenda || '-'} / {animal.loteNome}</td><td>{animal.qtd}</td><td>{formatarNumero(animal.p_ini)} kg</td><td>{formatarNumero(animal.p_at)} kg</td><td>{animal.status}</td><td><div className="row-actions"><button className="action-btn" onClick={() => editarAnimal(animal)}>Editar</button><button className="action-btn action-btn-danger" onClick={() => excluirAnimal(animal.id)}>Excluir</button></div></td></tr>)}</tbody></table></div>)}
      {abaAtiva === 'grupos' && grupos.length === 0 && <div className="animais-empty-state"><strong>Nenhum grupo cadastrado.</strong></div>}
      ...
      {abaAtiva === 'movimentacoes' && (historicoSaidas.length ? <div className="table-responsive"><table className="data-table"><thead><tr><th>Data</th><th>Tipo</th><th>Lote</th><th>Quantidade</th><th>Observacao</th></tr></thead><tbody>{historicoSaidas.map((m) => <tr key={m.id}><td>{formatarData(m.data)}</td><td>{normalizarSaida(m.tipo)}</td><td>{m.loteNome}</td><td>{m.qtd}</td><td>{m.observacao || '-'}</td></tr>)}</tbody></table></div> : <div className="animais-empty-state"><strong>Nenhuma movimentacao registrada.</strong></div>)}
    </Card>

    {mostrarCadastro && <Card title="O que voce quer cadastrar?"><div className="animais-mode-actions"><Button onClick={() => abrirNovoPorModo('grupo')}>Grupo de animais</Button><p>Use para cadastrar varias cabecas juntas em um lote.</p></div><div className="animais-mode-actions"><Button onClick={() => abrirNovoPorModo('individual')}>Animal individual</Button><p>Use para acompanhar um animal especifico.</p></div></Card>}
