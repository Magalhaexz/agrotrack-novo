...
export function buildOperationalCreatePayload(table, record, userId) {
  ...
  if (normalizedTable === 'animais') {
    ...
    const payload = {
      ...
      nome: toNullableString(safe?.nome ?? safe?.identificacao),
      ...
      dias: toNullableInteger(safe?.dias),
      consumo: toNullableNumber(safe?.consumo),
      observacao: toNullableString(safe?.observacao),
      rendimento_carcaca: toNullableNumber(safe?.rendimento_carcaca),
      preco_arroba: toNullableNumber(safe?.preco_arroba),
      data_referencia: toNullableDateString(safe?.data_referencia),
      data_nascimento: toNullableDateString(safe?.data_nascimento),
      metadata,
      cloud_id: cloudUuid || undefined,
    };
    delete payload.id;
